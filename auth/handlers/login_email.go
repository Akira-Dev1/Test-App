...
func RequestLoginCode(w http.ResponseWriter, r *http.Request) {
	...
	if err := SetCode(req.Email, code, 5*time.Minute); err != nil {
...
func VerifyLoginCode(w http.ResponseWriter, r *http.Request) {
	...
	storedCode, err := GetCode(req.Email)
	if err != nil {
		http.Error(w, "code expired or not found", http.StatusUnauthorized)
		return
	}
	...
	DeleteCode(req.Email)

	// TODO: создать/обновить пользователя в MongoDB, сгенерировать JWT
